...
// 使用块级作用域避免 IIFE，支持更好的调试体验
{
         ...
         container.innerHTML = `
            <div style="display:flex; flex-direction:row; height:100%;">
                ...
                <div id="chipPanel" style="width:0; overflow:hidden; transition:width 0.3s ease; display:flex; flex-direction:column; border-left:1px solid #334155; background:rgba(15,23,42,0.5);">
                    ...
                    <div id="chipChart" style="flex:1; min-height:300px;"></div>
                </div>
            </div>
        `
    ...
    function renderChipDistribution(date) {
        ...
        const prices = bins.map(b => b.price.toFixed(2))
        ...
        const option = {
            ...
            tooltip: {
                ...
                formatter: function(params) {
                    ...
                    const bin = bins[idx]
                    if (!bin) return ''
                    return `价格: ${bin.price.toFixed(2)}<br/>筹码: ${bin.percentage.toFixed(1)}%`
                }
            },
            grid: { left: '28%', right: '8%', top: '10%', bottom: '10%' },
            xAxis: {
                type: 'value',
                axisLine: { lineStyle: { color: '#475569' } },
                axisLabel: { color: '#94a3b8', fontSize: 9, formatter: (v) => v.toFixed(0) },
            ...
            series: [{
                ...
                itemStyle: {
                    color: function(params) {
                        const idx = params.dataIndex
                        const price = bins[idx]?.price || 0
                        return price <= currentClose ? '#ef4444' : '#10b981'
                    },
